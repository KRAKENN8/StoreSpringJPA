...
public class PurchaseServiceImpl implements PurchaseService {
    ...
    public boolean add() {
        try {
            ...
            System.out.printf("Покупатель ID: %d, Баланс: %.2f%n", c.getId(), c.getBalance());
            System.out.printf("Товар ID: %d, Цена: %.2f, Количество на складе: %d%n", p.getId(), p.getPrice(), p.getQuantity());
            System.out.printf("Запрашиваемое количество: %d, Общая стоимость: %.2f%n", qty, totalCost);
    ...
    @Override
    public boolean update() { // Метод без параметров
        return false;
    }
